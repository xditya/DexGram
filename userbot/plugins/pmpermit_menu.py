    ...
    if not pmpermit_sql.is_approved(chat_id):
        ...
        async with borg.conversation(chat) as conv:
         ...
         if y == "1":
             ...
             x = response.text
             if x == "1":
                 await borg.send_message(chat, "**Hey Girl. Please drop whatever you want to and donot try to flood my master's inbox otherwise you will be blocked.**")
                 response = await conv.get_response(chat)
                 if not response.text == "/start":
                     await borg.send_message(chat, LWARN)
                     response = await conv.get_response(chat)
                 ...
                 await borg.send_message(chat, "**Hey Bro. Please drop whatever you want to and donot try to flood my master's inbox otherwise you will be blocked.**")
                 response = await conv.get_response(chat)
                 if not response.text == "/start":
                     await borg.send_message(chat, LWARN)
                     response = await conv.get_response(chat)
